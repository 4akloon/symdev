...
| `has-thread-local` | `false` | Symbian TLS is `UserSvr::DllTls` calls, not an ELF TLS segment — and it stays `false` even now that `thread_local!` works (experiment 88), because `symdev-elf2e32` keeps only `PT_LOAD` program headers, so a `PT_TLS` segment would be silently dropped |
...
| 76 | **TLS and time**: `UserSvr::DllTls` behind `thread_local!`; `Instant`, `SystemTime`, with monotonic-versus-wall-clock stated honestly | **time: done** (2026-09-20, experiment 85): `symbian_std::time` gives `Duration`, `Instant`, `SystemTime`, `UNIX_EPOCH` and `SystemTimeError`, and `examples/time` reports 29 passing cases from inside the emulator — including the criterion, a `User::SetUTCTime` hour that moves `SystemTime` and leaves the `Instant` at 0. `Instant` is `User::TickCount` + `UserHal::TickPeriod` and **not** `NTickCount`, whose period no call on this link line will state; `SystemTime` is `UniversalTime` and not `HomeTime`, at an epoch measured from euser's own calendar because the computed one is 12 days wrong. **TLS: done** (2026-09-21, experiment 88): `thread_local!` over one `UserSvr::DllTls` slot holding a per-thread table keyed by each static's own address; destructors run at thread exit; 97 ns an access. **Step 77 is unblocked.** |
...
`std` needs, pervasively: `alloc` (68, done), atomics and `Mutex`/`Once` (72), **TLS** (76 — done, experiment 88; it was called the largest risk here because
`thread_local!` had to land on a facility nobody had touched, and it turned out not to be
one: `Dll` does not exist in this SDK at all, the calls are `UserSvr::DllTls` and friends,
an EXE gets at least 64 per-thread slots, and one access costs 97 ns against 162 for an
atomic), `time` (76, **done**), `fs`/`io` (71, done,
