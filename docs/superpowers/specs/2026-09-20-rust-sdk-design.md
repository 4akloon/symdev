...
- Async: **shipped, experiment 87.** A shim `CActive` subclass whose `RunL` calls a Rust
  `extern "C"` callback, and `symbian-async`, a single-threaded executor on the active
  scheduler. `block_on` owns a `CActiveScheduler`; `spawn` joins the one CONE already
  installed, because a GUI application may never own one (`avkon-rust-spec.md` §1.3).
  The waker is an `Arc` whose only state is an `AtomicU32`, so waking is one atomic
  store and the `Send + Sync` a `Waker` claims is true. The blocking twin —
  `symbian_core::net::blocking`, which step 74's whole `std::net` is built on — refuses
  to run on a thread that has a scheduler, because `User::WaitForRequest` and an active
  scheduler share the thread's request semaphore and would eat each other's completions.
...
| Build backend (`std`) | **Exp 89:** `[language] name = "rust-std"` is the same `RustBuild` with two differences and no others — `-Zbuild-std=std,panic_abort` instead of `core,alloc`, and `__CARGO_TESTS_ONLY_SRC_ROOT` pointing at the tree `StdSrc::materialise` built from the toolchain's `rust-src`, `crates/symbian-sys` and `symbian-rs/rust-src/overlay/`. The link line, the post-linker, the resources and the packaging are byte-for-byte the `rust` path's |
...
| 73 | **Async**: a `CActive` subclass in the shim whose `RunL` wakes a Rust waker, a single-threaded executor on `CActiveScheduler`; `examples/async` awaits an `RTimer` | **done** (2026-09-21, experiment 87): `examples/async` reports **15 passing** cases through `symdev test --emulator`, and the criterion is a measured figure — two 300 ms sleeps awaited together take **312 ms** against **625 ms** for the same two in sequence, with the shorter of a concurrent 400/100 pair recording itself first. One C++ file, `shims/common/symrs_active.cpp`, for rule 3 (`RunL`/`DoCancel` are pure virtual) and one `TRAP` in it, around `CActiveScheduler::Start()`; `RTimer` and `CActiveScheduler::Current()` are called from Rust directly. `symbian_async` gives `block_on` (installs a scheduler, polls the future on its own stack) and `spawn` (joins the scheduler that is already there — the only thing a GUI application may do), plus `sleep`, `join`, `race` and the `Request`/`Source` pair a second asynchronous source needs. The waker holds **nothing but an `AtomicU32`**, so `Send + Sync` is true rather than asserted. The two models are kept apart by the platform's own rule: `symbian_core::net::blocking` refuses with `KErrInUse` on a thread that has a scheduler. `corpus/87-async/`, 21 659 bytes, no new thread and no new DSO |
...
| 75 | **UI**: the Avkon app framework — `CAknApplication`/`CAknDocument`/`CAknAppUi`/`CCoeControl` are C++ classes with virtual methods, so the shim must *define the subclasses* and forward each virtual to a Rust function pointer; `examples/ui` draws and handles a key | **done** (2026-09-21, experiment 86): `shims/s60/symrs_avkon.cpp` forwards seven virtuals to a Rust vtable behind one exported symbol, `crates/symbian-ui` is the safe side (`App`, `Gc`, `Rect`, `Rgb`, `KeyEvent`, the key tables) reached as `symbian_std::ui`, and `#[symbian_std::main(gui)]` writes the vtable export and **no** `E32Main` — the shim owns it and no `CActiveScheduler` is created. A new `[ui]` manifest section makes `RustBuild` generate `<app>.rss` and `<app>_reg.rss` text with `localisable_resource_file`/`localisable_resource_id` and compile them with the existing byte-verified `cpp`+`rcomp`, plus `<app>_aif.mif` from `[symbian] icon`; a console project links and generates none of it. `uidemo.exe` is **12 715** bytes (7 559 without the result-file harness) against experiment 76's 107 028, and every other example is unchanged to the byte. Screenshots either side of `emukey.py keys <pid> Up Up`: three bars → five, 3 767 pixels changed. `symdev test --emulator` reports 3 passed. `symbian-rs/corpus/86-ui/`<br><br>**Menu and softkeys, 2026-09-21, experiment 91** (`symbian-rs/corpus/91-ui-menu/`): `[ui]` grew `[[ui.menu]]`, `left_softkey` and `right_softkey`, and the generated `.rss` now carries the application's **own** `CBA`, `MENU_BAR` and `MENU_PANE` beside the caption. `F1 Down Return F2` into `examples/ui` opens the Options menu, moves the highlight, runs the command and ends the process — 36 270 / 11 748 / 36 220 pixels of the 240×325 screen, `uidemo.exe` 12 844 with every other example unchanged to the byte. **The softkey that did nothing for three sessions was answered rather than worked around**: it always reached `HandleCommandL`, but the ROM's `R_AVKON_SOFTKEYS_EXIT` sends 3001 (`EAknSoftkeyBack`) while drawing `Exit`, and this spec's own table had `EAknSoftkeyExit` at 3002 where `avkon.hrh` puts it at 3009. The probes that should have shown this were invisible because EKA2L1's stock log filter has `Emulated.Stdout:off`. The fix is to stop naming a ROM CBA resource: the right button carries `EEikCmdExit` and the left `EAknSoftkeyOptions`, both header constants. A menu item's command id is `0x4000 | (FNV-1a-32(id) & 0x3fff)`, computed identically by `symdev-manifest`'s `CommandId` and `symbian-ui`'s `Command::named`, so the manifest and the Rust `match` share the *word* and neither writes the number.<br><br>**The menu left the manifest, 2026-09-21, experiment 95** (`symbian-rs/corpus/95-runtime-menu/`, design [runtime-menu-design.md](../../research/runtime-menu-design.md)): the rule is *the manifest holds what the phone needs before the application runs*, and an Options menu is only ever needed while it runs. `[[ui.menu]]`, `CommandId`, `Command` and `App::command` are **deleted**, and with them the generated `menu` module of constants and `symbian-macros`' dependency on symdev's manifest reader. An application writes `fn menu(&self, m: &mut Menu<Self>) { m.item("More bars", |app| app.bars += 1); m.exit("Exit"); }` — no id, no constant, no number. The compiled `MENU_PANE` is **empty** and every line is added through `CEikMenuPane::AddMenuItemL` from a new `CShimAppUi::DynInitMenuPaneL`, which fires for an empty pane (observed). The closure reaches the application because a non-capturing closure is a `fn(&mut A)`: the menu is declared a second time when a command arrives, the pointer is copied out, the `&self` borrow ends, and only then is it called with `&mut` — and an action is handed nothing but the application, so it cannot re-enter the framework. A label is cut at `ENominalTextLength` (40) on a character boundary, because `TBuf<40>::Copy` of anything longer is a descriptor *panic* no `TRAP` catches. `softkeys = "options-exit"` is now also how an application says it has a menu, and the manifest's old refusal of that value without a menu is replaced by emitting the menu bar from the same condition, so the access violation it guarded (re-verified: `KERN-EXEC 3`) cannot be expressed. `F1 Down Return` gives `bars=2 keys=0 cmd=1`, experiment 91's result through a closure. `uidemo` 12 844 → **13 714**; three menu-less GUI examples grew 284–329 bytes, which is the new shim code in a translation unit built without `-ffunction-sections`. |
| 76 | **TLS and time**: `UserSvr::DllTls` behind `thread_local!`; `Instant`, `SystemTime`, with monotonic-versus-wall-clock stated honestly | **time: done** (2026-09-20, experiment 85): `symbian_std::time` gives `Duration`, `Instant`, `SystemTime`, `UNIX_EPOCH` and `SystemTimeError`, and `examples/time` reports 29 passing cases from inside the emulator — including the criterion, a `User::SetUTCTime` hour that moves `SystemTime` and leaves the `Instant` at 0. `Instant` is `User::TickCount` + `UserHal::TickPeriod` and **not** `NTickCount`, whose period no call on this link line will state; `SystemTime` is `UniversalTime` and not `HomeTime`, at an epoch measured from euser's own calendar because the computed one is 12 days wrong. **TLS: done** (2026-09-21, experiment 88): `symbian_std::thread_local!`, `LocalKey`, `with`/`try_with` and `const { … }` initialisers, and `examples/tls` reports **45 passed**. The premise was wrong — **there is no `Dll` class in this SDK** and the whole surface is five `@internalAll` `UserSvr` statics — and so was the fear: an EXE gets **many** slots, keyed by a `TInt` it chooses, and they are **per thread** (64 held at once; a worker reads null where its creator stored a value). The SDK still takes **one** slot and puts its own table behind it, because the kernel will not enumerate a thread's slots and `Drop` needs that list. **One access is 100 ns, of which the kernel call is 53; `AtomicU32::fetch_add` beside it is 158** — a thread-local is the cheap per-thread state on this device. Destructors run at the end of every `spawn`ed thread; the main thread calls `thread::drop_thread_locals()` itself, which is step 77's `lang_start`'s job. |
| 77 | **`std` for `target_os = "symbian"`** — the owner's decision, 2026-09-20. Fork `rust-src`, add `library/std/src/sys/pal/symbian` over the crates below, build it with `-Zbuild-std=std`. The facade's `fs`/`io`/`sync`/`thread` are re-hosted there rather than rewritten, and `#[symbian_std::main]` becomes std's `lang_start`. | **done** (2026-09-21, experiment 89): an application with **no `#![no_std]`** reports **20 passing** cases through `symdev test --emulator`, and ten crates from crates.io compile unchanged. It is **not a fork**: `symbian-rs/rust-src/overlay/` is twenty-odd files, the other 82 MB come from the toolchain at build time, and `StdSrc` materialises the two together and points cargo at the result with `__CARGO_TESTS_ONLY_SRC_ROOT` — `rustup toolchain link` cannot do it, because a symlinked `bin/rustc` resolves `/proc/self/exe` back to the real sysroot. Each replaced file carries the SHA-1 of the toolchain original, so a nightly bump stops the build and names the file. `std` cannot depend on the SDK crates (`error[E0463]: can't find crate for 'core'`), so `symbian-sys` is **copied into** `library/` with a manifest naming `core` by path, as `rustc-std-workspace-core` does; everything above it was written into `sys/*/symbian`. `println!` goes to `E:\symdev\stdout.txt`, not to `User::InfoPrint`, which is a dialog and not a stream; a panic still reaches the screen, through `User::Panic(_L("RUST"))`. The entry point is `std::os::symbian::start` (`lang_start` plus the main thread's thread-local sweep) behind the unchanged `#[symbian_std::main]`. Every `no_std` example is unchanged to the byte. `symbian-rs/corpus/89-std/`<br><br>**Gaps closed 2026-09-21, experiment 90** (`symbian-rs/corpus/90-std-gaps/`): `stdhello: 50 passed` (73 633 bytes) and a second example, `examples/std-net`, `stdnet: 31 passed` (53 359). **Real now, in addition:** a drive letter as a `Path` prefix (`Prefix::Disk`, with Symbian's *single* session path — not a per-drive one — as the reason `E:x` is still relative); `read_dir` over `RFs::GetDir`/`CDir`; `args` over `User::CommandLine` with `RProcess().FileName()` as element 0, split on whitespace because this platform states no quoting convention; **`std::net`** — TCP connect/listen/accept/read/write/shutdown, UDP, and `RHostResolver` behind `ToSocketAddrs`, inheriting step 73's rule that a blocking call **refuses** under a `CActiveScheduler` (`KErrInUse`) instead of hanging, which the example verifies by installing one; **`std::process`** — spawn, wait, exit code, `kill`, `id`, and `std::process::exit` as `User::Exit`; and a **`CTrapCleanup` on every thread**, which nothing installed before and without which any framework call that touches the cleanup stack panics `E32USER-CBase 69`. **Still `Unsupported`, each for a stated reason:** `env` (Symbian has no environment; a process-local map would be a lie the first child exposes); `std::path::absolute` (needs `RFs::Parse`/`TParse`, unmeasured); socket timeouts, `set_nonblocking(true)`, `peek`, `try_clone`, every socket option and IPv6; `Stdio::piped()`, `Command::output()` and `current_dir` — **`RPipe` is 9.4 and is not in this SDK**, verified by grepping every header. |

**Step 77 is where `#![no_std]` disappears, and it is gated on real work, not on a rename.
Every gate is now open — and every one of them held: `alloc`, the atomics, TLS, `time` and
`fs` were all re-hosted rather than rewritten, exactly as this paragraph predicted, and the
one that did not close was `net`, which is still reached through the facade.** `std` needs, pervasively: `alloc` (68, done), atomics and
...
Dependencies that fix the order: **"73 before a real 75" was wrong too, and step 75
disproved it.** The app framework does run an active scheduler — CONE's own
`CCoeScheduler` — but that is a reason for the application never to *create* one, not a
reason to need an executor first: a view that draws and handles keys is called *by* the
running scheduler and awaits nothing. Step 75 needs nothing of 73, and would have been
buildable before it. What 73 gives an Avkon application is work *between* callbacks —
and the one thing still unproven on either side is 73's executor running on **CONE's**
scheduler rather than on the one `block_on` installs (experiment 87's own open item).
**73 before 74 was wrong, and step 74 disproved it**: every socket operation is
...
~~Full `std`~~ — shipped in step 77, **except `std::net`**, which stays a non-goal for this
phase: sockets work through `symbian_std::net` (step 74) and `sys/net/symbian` is a slice of
its own. The Avkon **view architecture** (the single-view
application landed in step 75 and its Options menu in 95, `AVKON_VIEW` did not); TLS in the
*transport* sense — `securesocket.dso`, which step 74's plain
