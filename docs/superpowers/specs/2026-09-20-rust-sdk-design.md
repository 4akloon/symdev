...
| 76 | **TLS and time**: `Dll::Tls`/`UserSvr` behind `thread_local!`; `Instant`, `SystemTime` over `User::NTickCount` and `TTime::HomeTime`, with monotonic-versus-wall-clock stated honestly | `thread_local!` holds a value per `RThread`, and a monotonic duration survives a device clock change |
...
`std` needs, pervasively: `alloc` (68, done), atomics and `Mutex`/`Once` (72), **TLS** (76 — the
largest risk, since `thread_local!` has to land on `Dll::Tls`), `time` (76), `fs`/`io` (71, done,
