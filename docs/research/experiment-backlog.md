...
## 91. The Options menu, and why a softkey did nothing in our applications (T5, Rust SDK)

- **Requires:** experiments 83 (keys into the emulator) and 86 (the Avkon shim, `symbian-ui`, the `[ui]` resource stage). Design: [avkon-rust-spec.md](avkon-rust-spec.md) §2.3, §5.2, §6.3, [2026-09-20-rust-sdk-design.md](../superpowers/specs/2026-09-20-rust-sdk-design.md) §11 step 75.
- **Why:** three sessions had recorded "softkeys do nothing in our own applications" as unknown. F1/F2 are shipped to the focus window group and drive a ROM application's menu and Exit, but `examples/gui` with `R_AVKON_SOFTKEYS_EXIT` did not exit and neither did the Rust `examples/ui`. A menu is opened by the left softkey, so a menu could not even be tested until this was understood.
- **Procedure:** (1) instrument the shim — `RDebug::Print` in `CShimAppUi::HandleWsEventL`, `HandleCommandL` and `CShimView::OfferKeyEventL` — and find where the key stops; (2) read the command ids out of `avkon.hrh` rather than recalling them; (3) decide where the command ids of a generated application come from; (4) grow `[ui]` a menu, generate `CBA`/`MENU_BAR`/`MENU_PANE` into the existing byte-verified `cpp`+`rcomp` path; (5) `Command` in `symbian-ui` so an application matches by word; (6) drive `F1 Down Return F2` into `examples/ui` and diff the pixels.
- **Outcome:** pass (emulator only; no device)
- **Evidence:** 2026-09-21, `symbian-rs/corpus/91-ui-menu/` (`uidemo.exe` 12 844, the generated `uidemo.rss`/`uidemo.rsc` and four screenshots); source is `symbian-rs/examples/ui`.

  - **The softkeys were never dead, and the first thing in the way was the log.** EKA2L1's stock `log-filter` carries **`Emulated.Stdout:off`** (`~/.local/share/EKA2L1/config.yml`; the preset is in `config/include/config/config.h`), and `RDebug::Print` lands in exactly that class — `kernel/src/svc.cpp`'s `debug_print` does `LOG_TRACE(EMULATED_STDOUT, …)`. Every guest print from every probe in three sessions went into that hole. This is the second time a default log filter has cost a session: experiment 90 lost three hours to `Kernel:Warn` hiding a panic. **Set `Emulated.Stdout:trace` before instrumenting the guest.**
  - **A softkey reaching `OfferKeyEventL` would be the bug.** The button group container is on the control stack at `ECoeStackPriorityCba` = 60 and the application's view at `ECoeStackPriorityDefault` = 0 (`coeaui.h:47-61`), so the CBA is offered the `EEventKey` first and consumes it. The view sees the `EEventKeyDown`/`Up` pair and nothing else. "F2 never reaches `OfferKeyEventL`", recorded in experiment 83 as a symptom, was evidence that the CBA was **working**.
  - **The command that arrived was 3001.** One F2 into the *unchanged* `examples/ui`: `ws type=3 code=0 scan=a5` (offered to the view, declined), `ws type=1 code=f843 scan=a5` (consumed by the CBA), then `HandleCommandL 3001`. The shim compares `aCommand` against the symbols `EEikCmdExit` and `EAknSoftkeyExit`, so it was looking for 0x100 and 3009. **The shim's code was never wrong; `avkon-rust-spec.md` §5's table was** — it said `EAknSoftkeyExit = 3002`, recalled rather than read, and `avkon.hrh:330-339` puts the enum at Options 3000, **Back 3001**, Mark 3002, Unmark, Insert, Yes, No, Done, Close, **Exit 3009**.
  - **So the ROM's `R_AVKON_SOFTKEYS_EXIT` draws `Exit` and sends `EAknSoftkeyBack`.** The id is `0x8cc002a` in the SDK's `avkon.rsg` and the ROM's `avkon.r01` does carry offset `0x8cc0`, so the file is the one being read; *why* resource 42 in it disagrees with the SDK's name was **not determined** — it is dictionary-compressed and was not decoded, and the device profile says `epoc93fp2`, the same platform as the SDK. **Unexplained, and it no longer matters.**
  - **The fix is to own the resource, not to add a constant.** A GUI project's generated `.rss` now declares its own `RESOURCE CBA r_symrs_cba`: `EEikCmdExit` (`eikon.hrh:376`) on the right button, `EAknSoftkeyOptions` on the left. Both are compile-time constants out of SDK headers, so no ROM resource's numbering can reach them. `EAknSoftkeyOptions` is the one id that may **not** be replaced by a symdev-chosen number — the framework itself watches for it and opens the menu bar instead of forwarding it, which is also why `softkeys = "options-exit"` with no `[[ui.menu]]` gives `Access violation reading address 0x9C in thread Bars` on F1, with no `HandleCommandL` first. The manifest refuses that combination and says why.
  - **There is no `AVKON_MENUBAR` struct.** The resource structs are `MENU_BAR`, `MENU_TITLE`, `MENU_PANE` and `MENU_ITEM` (`eikon.rh:97-128`) and `CBA`/`CBA_BUTTON` (`eikon.rh:335-350`); `avkon.rh` has only `AVKON_MENU_LIST`, which is something else. `CBA_BUTTON`'s `id` is a **`WORD`**, which is what bounds a command id at 16 bits.
  - **Command ids come from the name, on both sides, and the number is written nowhere.** A menu lives in the compiled `.rss` and in a Rust `match`, which never meet, so `symdev-manifest`'s `CommandId::of` and `symbian-ui`'s `Command::named` both compute `0x4000 | (FNV-1a-32(name) & 0x3fff)` — the manifest says `id = "more"`, the source says `Command::named("more")`, and the word is the only thing repeated. The range is bounded at both ends deliberately: below `0x4000` is everything the platform names (`EEikCmd*` at `0x100`, `EAknSoftkey*` at 3000–3200, the reserved softkey ranges at `0x1000`/`0x1100`/`0x1200`), and `0x8000` is where a `WORD` id runs out, so one range serves a menu item and a softkey alike. Two names that hash alike are a build error naming both. The two implementations are pinned to one another by one shared vector table — a `#[test]` on the host and a **`const` assertion** on the target, because that workspace builds for the phone and `cargo test` never runs in it.
  - **`rcomp` resolves a forward `LLINK`.** `EIK_APP_INFO` has to stay the third resource, so it names `r_symrs_menubar` and `r_symrs_cba` before either is declared — the layout every hand-written S60 `.rss` uses. Our native resource compiler handles it unchanged; nothing new encodes a resource.
  - **The whole sequence, in pixels.** `emukey.py keys <pid> F1 Down Return F2`, one PID-bound screenshot after each, diffed over the emulated screen (240×325 at (330,137) in the 900×600 window). Start: `bars=3 keys=0 cmd=0` with **Options** and **Exit**, both our own labels. **F1**: the Options menu opens with Avkon's "Show open apps." above our *More bars / Fewer bars / Reset / Exit* — **36 270 of 78 000** pixels, bbox (0,20)–(240,319). **Down**: the highlight moves one line — **11 748**, bbox (5,179)–(223,233). **Return** (`EStdKeyDevice3`, `scan=a7`): the menu closes and the drawing reads `bars=2 keys=0 cmd=1`, and the probe build logs `HandleCommandL 30226` = `0x7612` = `Command::named("fewer")` — **36 220**, bbox (0,145)–(240,319). Over the client area alone, start → selected is **1 481 of 58 800**, bbox (63,146)–(212,227). **F2**: the process is gone. `grep -c 'Panic\|KERN-EXEC'` on the log is 0, and `keys=0` throughout is the proof that no softkey ever touched `OfferKeyEventL`.
  - **Sizes.** `ui` **12 844** (was 12 715: the menu resource is in the `.rsc`, not the EXE; the 129 bytes are the `command` callback, four `Command` constants and `cmd=` in the drawing). Every other `no_std` example is unchanged **to the byte**: `hello-raw` 752, `hello` 3 187, `alloc` 4 474, `shim` 4 474, `files` 10 552, `atomics` 11 719, `net` 13 379, `tls` 16 272, `time` 20 583, `async` 21 659. `uidemo.rsc` grew 140 → **364** bytes. `std-hello` measures **73 617** here, 16 under experiment 90's 73 633 — measured twice, once with this change and once with `symbian-ui` checked out at `main`, identical both times (`.text` 124 668), so the difference is not this step's.
  - **`symdev test --emulator` still reports `uidemo: 3 passed`**, unchanged: it tests the entry path, and the screenshots test the menu.
  - **Not observed, and left as such.** Why the ROM's `R_AVKON_SOFTKEYS_EXIT` sends `EAknSoftkeyBack`. Whether a non-ASCII caption, softkey label or menu label survives `cpp`+`rcomp` and reaches the screen — the manifest accepts one, the generator escapes it, and nothing but ASCII has been through the compiler. Cascading menus, `MENU_ITEM` flags, checkboxes and the middle softkey. A menu declared at run time rather than in the resource. And, as for every step so far, **no device.**
  - **One run did not reproduce.** An early `F1 Down Return` left the application at `bars=6 keys=6 cmd=2` where the identical sequence gives `bars=2 keys=0 cmd=1` before and after; extra key events, not diagnosed, seen once in four runs of that sequence. Worth knowing because it can make a correct build look broken.
## 94. The S60 list box: which class, what a row really is, and who owns the item array (T5, Rust SDK)

- **Requires:** experiments 76, 77, 81, 83 (keys into the emulator), 86 (the Avkon shim, the vtable and `examples/ui`). Design: [avkon-rust-spec.md](avkon-rust-spec.md) §3 (the forwarding ABI), §4.3 (the per-virtual leave rule), §5.2 (keys).
- **Why:** a list box is the central component of every Avkon application and the first one that has to *embed* in the view step 75 built. It is also the slice where the SDK is easiest to get wrong from recollection: the row format, the item-array ownership and the key precedence each have a wrong answer that produces blank rows, a double free or an application that never sees a selection.
- **Procedure:** (1) settle the class, the row format, the ownership rule, the scroll-bar calls and the observer from `aknlists.h`/`eiklbx.h`/`eiktxlbm.h`/`eiklbo.h`/`eiksbfrm.h`/`coeaui.h` and `nm -D`, not from memory; (2) `symbian-rs/shims/s60/symrs_list.{h,cpp}` — the list, its item array and `MEikListBoxObserver`, behind plain C entry points; (3) `symbian-rs/crates/symbian-ui/src/list/` — `List` and `Rows`, no `unsafe` for the application; (4) `symbian-rs/examples/ui-list` through `symdev build`/`package`/`run`/`test --emulator`, with PID-bound screenshots either side of `docs/research/acceptance/emukey.py`.
- **Outcome:** pass (emulator only; no device)
- **Evidence:** 2026-09-21, `symbian-rs/corpus/94-ui-list/` (the 14 489-byte E32 and four screenshots); source is `symbian-rs/examples/ui-list`.

  - **`CAknSingleStyleListBox`, and the header says what a row is.** `aknlists.h` line 224, the `list_single_pane` style — the simplest of the single-line family that shows text and needs no icon array. Its own doc comment (lines 216–222) gives the format: `list item string format: "\tTextLabel\t0\t1"` / `where 0 and 1 are indexes to icon array`. So a row is TAB-separated columns — an empty leading column, the label, then two icon-array indices. This shim sets no icon array and therefore writes **`"\t" + label`** and stops; writing `0` and `1` would index an array that does not exist, and what a column list box does with that was not observed. The rows render correctly, left-aligned at the text column, in all four screenshots. Every other single-line style (`CAknSingleGraphicStyleListBox`, `CAknSingleNumberStyleListBox`, `CAknSingleHeadingStyleListBox`) puts an icon index or a second string in column 0, so each would need a `CAknIconArray` there is nothing to put in.
  - **The shim owns the item array, and that is the answer that is not a double free.** `CTextListBoxModel::SetItemTextArray` (`eiktxlbm.h`) only *assigns* — it does not delete what was there — and `TListBoxModelItemArrayOwnership` (`eiklbm.h` line 26) is `ELbmOwnsItemArray = 0` / `ELbmDoesNotOwnItemArray = 1`. `CSymRsList` allocates one `CDesCArrayFlat`, hands the model a borrow of it with `ELbmDoesNotOwnItemArray`, and every later change `Reset()`s and refills *that same array*. Ownership never changes hands, so the non-deleting assignment can neither leak nor double-free, and `~CSymRsList` deletes the list box first and the array second.
  - **Key precedence is a documented priority, not an insertion order.** `coeaui.h` lines 36–38: "Controls with higher priorities get offered key events before controls with lower priorities." `symrs_avkon.cpp` stacks `CShimView` at `ECoeStackPriorityDefault` (= 0, `coeaui.h` line 47), so the list goes on at `ECoeStackPriorityDefault + 1` and **the list wins**. Observed: three `Down` presses move the highlight and the application's `App::key` sees none of them. That is the intended division — an application reacts to a selection, not to an arrow — and everything the list declines still reaches the view.
  - **The list is its own stacked control, and that is what kept `symrs_avkon.cpp` untouched.** `CShimView` returns 0 from `CountComponentControls` and does not forward it to Rust, so a windowless child of the view would have needed a change to the app-framework shim another slice owns. Instead the list is built with `CEikTextListBox::ConstructL(NULL, … | CEikListBox::ECreateOwnWindow)` — `eiklbx.h` line 1042 says the parent "may be NULL", and `ECreateOwnWindow` is `0x0200` (`lafpublc.h` line 154) — and added to the app UI's own control stack beside the view. **The only thing it needs from the rest of the crate is the `CShimAppUi*`**, which is one `pub(crate)` accessor on `Ui`; it needs no shared "control on the stack" abstraction, and `symrs_avkon.{h,cpp}` are not edited at all.
  - **A selection reaches Rust through the observer, under the same leave rule as every other virtual.** `MEikListBoxObserver::HandleListBoxEventL(CEikListBox*, TListBoxEvent)` (`eiklbo.h`) is an ordinary C++ interface with one pure virtual, so `CSymRsList` implements it and forwards. `TListBoxEvent` has no explicit values, so `EEventEnterKeyPressed = 0` and `EEventItemClicked = 1`; those two are "the user chose this", the rest are editing and dragging. The Rust frame returns a `TInt` and the shim calls `User::LeaveIfError` **after** it has returned, on a stack that is pure C++ again (spec §4.3).
  - **The proof that the application saw the right index is 82 pixels.** A full-screen list covers the view, so `App::draw` is invisible and the only surface a selection can show itself on is the list. `Return` on row 3 rewrites row 0 from `picked: -` to `picked: 3`: **82 of the client area's 59 280 pixels change, bounding box (77,24)–(85,35)** — a 9x12 box holding exactly the one glyph. The three `Down` presses before it changed 12 800 pixels, box (1,14)–(229,125). Reproduced identically on two separate builds and runs.
  - **Scroll bars work and are tracking the model.** `CreateScrollBarFrameL(ETrue)` then `SetScrollBarVisibilityL(EOff, EAuto)` (`eiksbfrm.h` line 107: `EOff = 0`, `EOn = 1`, `EAuto = 2`) — never a horizontal bar, a vertical one as soon as the items overflow. With thirteen rows and nine visible the thumb is **138 pixels of a 246-pixel shaft** and travels from y 57–194 to y 146–283 as the list scrolls, keeping its height.
  - **`CCoeControl::Rect()` is window-relative on a window-owning control, and it cost a build to find out.** The first version took the list's rectangle from the view's `Rect()`, on the reasoning that `CShimView` was constructed with `ClientRect()`. The list came up **over the title pane**: a window-owning control reports an origin of `(0, 0)`, which is the same ambiguity spec §5.1 left open for `Draw` ("two different rects produced identical pixels") and this is the case that separates them. The shim now takes `CEikAppUi::ClientRect()`, which is also what `aknlists.h` lines 208–210 require of this whole family ("the `Rect()` of the list must be `ClientRect()`"). After the change the title pane reads `List`, the manifest's `short_caption`. **The Rust `List` no longer receives the view handle at all**, so no coordinate crosses the boundary.
  - **`eikcoctl.dll` and `eikctl.dll`, and they belong in the `--as-needed` group.** `nm -D` splits the list across three DLLs, not one: the `CAkn*StyleListBox` leaves are `avkon`'s, but `CEikListBox`, `CEikTextListBox`, `CTextListBoxModel` and `CEikScrollBarFrame` come from **`eikcoctl.dll`**, and `CEikColumnListBox`'s own `ConstructL` and `Model` — which is what a `CAknSingleStyleListBox` really calls, because the subclass redeclares them and hides `CEikTextListBox`'s — from **`eikctl.dll`**. Putting them in `RustSdk::UI_LIBRARIES` **was wrong and was caught by `readelf -d`**: that list is emitted outside the `--as-needed` group, so `examples/ui`, a GUI application with no list at all, recorded both as `DT_NEEDED` and would have loaded two DLLs to call nothing. They are in `RustSdk::LIBRARIES` instead, beside the `bafl.dso` the item array needs, and `uidemo` now records neither.
  - **The list loop-scrolls, which nothing asked it to do.** `EAknListBoxSelectionList` is `EAknGenericListBoxFlags` is `EAknListBoxScrollBarSizeExcluded` = `0x0080` (`avkon.hrh` lines 46, 64, 75) and carries no `EAknListBoxLoopScrolling` (`0x1000`), yet `Down` from the last row wraps to the first and `Up` from the first wraps to the last. Observed, not configured; the Avkon LAF is presumably doing it. Recorded rather than suppressed.
  - **Sizes**, all rebuilt in one tree after rebasing onto the menu, notes and query slices. `listdemo` **14 489**. Unchanged **to the byte** by this work: `uidemo` 12 844 (experiment 91's figure, not 86's 12 715 — the menu added 129 bytes before this branch rebased onto it), `notes` 15 298, `query` 20 120, `hello` 3 187, `hello-raw` 752. The `-l:euser.dso -l:drtaeabi.dso`-before-the-Rust-archive ordering is untouched, and the list needed **no second `-u`**: the one call that runs from C++ into Rust is a function pointer in a table passed at create time, not a symbol the shim imports, so only `symrs_app_vtbl` still needs one.
  - **`symdev test --emulator` passes six cases** beside the pixels, covering what a screenshot cannot: `construct` was reached, the client area is **240x245**, the model holds all thirteen rows, row 0 is current to begin with, the highlight can be moved from Rust and moved back, and an index past the end is refused. `grep -c 'Panic\|KERN-EXEC'` on the log is 0 throughout.
  - **What the application writes.** `List::new(ui, &items)?`, `set_items`, `selected() -> usize`, `set_selected`, `len`/`is_empty`, and `on_select(|index, rows| …)`. No `unsafe`, no Symbian type, text in as `&str`. `on_select` takes a `&mut Rows` rather than only the index because on a full-screen list the callback has to be able to rewrite the thing that owns it, and `Rows` borrows the control handle alone — which is what spares every application an `Rc`/`Weak` cycle. The closure is moved out of its box for the length of the call and put back, so a re-entrant event would find no closure instead of taking a second `&mut`; whether the list can report one was never observed either way.
  - **What this does not show:** a device; icons, headings, numbers or any style with an icon array; marking and multi-selection (`EAknListBoxMarkableList`); a list that is not full-screen, which `aknlists.h` forbids for this family and was therefore not tried; `HandleItemRemovalL`, the incremental-match buffer, `EditItemL` and pointer events; a list inside a dialog; a label longer than the 128-code-unit limit on a real screen; and a leave raised from inside the Rust callback, since nothing in `symbian-ui` can raise one.

## 95. The Options menu without the manifest: labels and closures declared in Rust, added from `DynInitMenuPaneL` (T5, Rust SDK)

- **Requires:** experiments 86 (the Avkon shim, the vtable and `examples/ui`), 91 (the Options menu in `symdev.toml`, the CBA fix, the access violation), 94 (the emulator loop for a stacked control). Design: [runtime-menu-design.md](runtime-menu-design.md); [avkon-rust-spec.md](avkon-rust-spec.md) §3.3, §4.3, §6, §7, corrections marked **[95]**.
- **Why:** experiment 91 put the menu in `symdev.toml` and matched it in Rust by hashing the same word on both sides; `command-id-design.md` then spent a branch making that word safe with a generated `menu` module of constants. The user's objection — «Навщо ці команди взагалі? Мені не подобається використання symdev.toml, він має бути лише для конфігурації проєкту» — is the design rule the whole thing was missing: **the manifest holds what the phone needs before the application runs**, and an Options menu is only ever needed while it runs. If the menu is declared in Rust there is no number for two compilers to agree on, so the entire problem the constants solved disappears with it.
- **Procedure:** (1) read `eikmenup.h`/`eikmobs.h`/`eikon.hrh`/`eikon.rh` for `AddMenuItemL`, `SData` and `DynInitMenuPaneL` (`LC_ALL=C grep -a`: `eikmenup.h` is non-ISO extended ASCII and the repo's grep skips it silently); (2) `Menu<A>` in `symbian-rs/crates/symbian-ui/src/menu.rs` with `item(label, fn(&mut A))` and `exit(label)`, two new ABI entries (`menu_item` down, `menu` up); (3) `DynInitMenuPaneL` and a trapped `HostMenuItem` in `symrs_avkon.cpp`; (4) delete `[[ui.menu]]`, `CommandId`, `MenuItem`, `Command`, `App::command` and the macro's `menu` module; (5) emit an empty `MENU_PANE` from the same condition that puts `EAknSoftkeyOptions` on the left button; (6) `examples/ui` through `symdev build`/`package`/`run` with `docs/research/acceptance/emukey.py`.
- **Outcome:** pass (emulator only; no device)
- **Evidence:** 2026-09-21, `symbian-rs/corpus/95-runtime-menu/` (the 13 714-byte E32, the generated `.rss` and four screenshots); source is `symbian-rs/examples/ui` and `symbian-rs/crates/symbian-ui/src/menu.rs`.

  - **`DynInitMenuPaneL` fires for an *empty* compiled `MENU_PANE`, and that is the whole design.** `rcomp` accepts `RESOURCE MENU_PANE r_symrs_menupane { items = { }; }` — an empty `items` array compiles — and `eikmenup.h:456` documents `AddMenuItemL` as adding an item "dynamically … Updates the menu's scroll bar to take account of the new item". Observed: F1 opens a pane holding Avkon's own **"Show open apps."**, which the framework inserts itself, above the four lines Rust added. `01-menu.png`.
  - **The resource cannot go away entirely.** Nothing calls `DynInitMenuPaneL` for a menu bar with no pane, and experiment 91's failure was re-verified on the current emulator build by forcing `UiResources::has_menu()` to `false` — `EAknSoftkeyOptions` on the left button, no `menubar` in `EIK_APP_INFO`: F1 gives `Thread Bars terminated … with category: KERN-EXEC and exit code: 3`, an access violation, with no `HandleCommandL` first. What stays compiled is a `MENU_BAR` with one `MENU_TITLE` and an empty `MENU_PANE`; every line is runtime.
  - **All of `SData` is set, from the resource compiler's own defaults.** The header insists — "NOTICE that `SData` is a structure so all fields in it should be set to avoid any unexpected behaviour" — and `eikon.rh:103` gives `STRUCT MENU_ITEM` `command=0`, `cascade=0`, `flags=0`, which is the same structure by another route. So a plain line is `iCascadeId = 0`, `iFlags = 0` (the flags are `eikon.hrh:221-239`: dimmed, separator, checkbox, radio, hotkey) and `iExtraText.Zero()`, which is where `CEikMenuPane` would otherwise show a hotkey name. Nothing is left uninitialised and nothing is guessed.
  - **A 40-code-unit label is a *panic*, not a leave, so it is cut in Rust before the descriptor sees it.** `SData::iText` is a `TBuf<ENominalTextLength>` with `ENominalTextLength = 40` (`eikmenup.h:76`), and `TDes16::Copy` of anything longer raises `ETDes16Overflow = 11` (`e32panic.h:131`) — no `TRAP` catches a descriptor panic. `App::menu` has no error channel (it returns nothing, like `Gc::text`), so `Menu::add` encodes as many **whole characters** as fit into a `[u16; 40]` stack buffer. The same `encode` compiled for the host: 47 units → **40**; a surrogate pair straddling the boundary → **39**, never split; 50 units of surrogate pairs → **40**. On the phone, with one item temporarily given a 47-character label: the pane rendered it ellipsised by Avkon, **the application did not panic**, and selecting it still fired the right action (`bars=3 keys=0 cmd=1`). `04-long-label.png`, `05-long-fired.png`. The shim clamps again at `ENominalTextLength` because the descriptor's invariant lives there; with the Rust cut it never has anything to do.
  - **The closure reaches the application because it is a `fn` pointer, and that is what makes it sound.** `App::menu(&self, m: &mut Menu<Self>)` is called **twice**: once from `DynInitMenuPaneL` to fill the pane, with each line's command set to `0x4000 + position`; and once from `HandleCommandL`, into a `Menu::find(position)` that adds nothing and only keeps the matching action. A non-capturing closure coerces to `fn(&mut A)` — `Copy`, borrowing nothing — so the pointer is copied out, **the `&self` borrow ends**, and only then is it called with `&mut A`. No `static mut`, no leak, no `Rc`, no boxed closure, and the two borrows are never live at once. Declaring the menu twice is what the observer asks for anyway: it fires every time the menu opens.
  - **Re-entrancy is impossible by construction, and that is why the repaint is the crate's.** An action is handed the application and **nothing else** — no `Ui`, no handle into C++ — so it cannot make a framework call and the framework cannot produce a nested callback. `vtbl::command` calls `Ui::redraw` after the action has returned; giving the action a `&Ui` instead would have handed it the way back in. For the same reason `m.exit(label)` is a separate line rather than a closure calling `Ui::exit`: it carries `EEikCmdExit`, which the shim acts on itself, so no Rust frame is on the stack while the framework tears the application down. Observed: `F1 Down Down Down Return` on it ends the application with no panic in the log.
  - **The acceptance run is experiment 91's, to the pixel and through a closure.** `symdev build && symdev package && symdev run`, then `emukey.py keys <pid> F1 Down Return`: the drawing reads **`bars=2 keys=0 cmd=1`** — three bars became two through `m.item("Fewer bars", |app| …)`. `00-start.png`, `01-menu.png`, `02-fewer.png`. `symdev test --emulator` still reports `uidemo: 3 passed` (`the view was sized before construct: 240x245`) and `grep -c 'Panic\|KERN-EXEC'` on the log is 0. (`symdev package` does **not** rebuild; running it without `symdev build` first ships the previous `.exe` and cost one emulator run.)
  - **The `options-exit` refusal was replaced, not deleted.** The manifest used to refuse `softkeys = "options-exit"` without `[[ui.menu]]`, and that refusal guarded the access violation above. With no menu in the manifest it cannot count items any more, so `UiResources` emits `menubar = r_symrs_menubar` and the `MENU_BAR`/`MENU_PANE` pair from the **same condition** that puts `EAknSoftkeyOptions` on the left button: one condition, so the two cannot disagree and the contradiction can no longer be written down. The default is now `"exit"`. `softkeys = "options-exit"` is therefore also how an application says it *has* a menu, and it has to be said in the manifest because the button group and the menu bar are compiled resources. What is still unchecked: `options-exit` with no `App::menu` gives a pane holding only "Show open apps." — a pane, not a crash.
  - **What was deleted.** `crates/symdev-manifest/src/command_id.rs` (`CommandId`, the FNV-1a hash, its vector table), `MenuItem`, `[[ui.menu]]`, the id-is-an-identifier rule and the constant- and hash-clash refusals; `symbian-rs/crates/symbian-ui/src/command.rs` entirely — `Command`, `Command::named`, `Command::is`, `Command::EXIT`, `Command::OPTIONS` and the target half of the vector table — and `App::command` with it; `symbian-rs/crates/symbian-macros/src/menu.rs`, `Entry::shape` and the `symdev-manifest` path dependency, so `symbian-rs/Cargo.lock` is back to **23 packages from 42** and `#[symbian_std::main(gui)]` reads no file again; and `docs/research/command-id-design.md`, which this supersedes. **Nothing of `Command` survives**: `EEikCmdExit` and `EAknSoftkeyOptions` are both handled below the crate, and `EAknSoftkeyEmpty` (what the left button sends under `softkeys = "exit"`) is outside the position range and ignored.
  - **Sizes**, every example rebuilt twice in one session with each branch's own `symdev`. `ui` 12 844 → **13 714** (+870): the labels and the closures moved out of a compiled resource and into the image, which is what putting the label next to its code costs. Three GUI examples with **no** menu still grew — `notes` 15 298 → **15 626**, `query` 20 120 → **20 449**, `ui-list` 14 489 → **14 773** — and that is `DynInitMenuPaneL` plus `HostMenuItem` in `symrs_avkon.cpp`: the recorded GCCE argv has no `-ffunction-sections`, so a translation unit's whole `.text` is one section (`libsymrs.a` 6 900 → **7 358** bytes of `.text`) and `--gc-sections` can only drop it whole. Splitting them out would not help, because the host table in the same file names `HostMenuItem`. Unchanged **to the byte**: `hello-raw` 752, `hello` 3 187, `alloc` 4 474, `shim` 4 474, `files` 10 552, `atomics` 11 719, `net` 13 379, `tls` 16 272, `time` 20 583, `async` 21 659, `spawnee` 3 208, `std-net` 53 495. `std-hello` 73 565 → 73 562, the crate-disambiguator layout wobble of experiment 87.
  - **What this does not show:** a device; a cascading submenu (`iCascadeId`) or a dimmed/checkbox/radio item (`iFlags`); a second menu bar or a second pane, which is the only reason `DynInitMenuPaneL`'s `aResourceId` would have to be checked and it is therefore ignored; `InsertMenuItemL`, `SetItemTextL` and the rest of the pane's mutating API; more than a handful of items, so the scroll bar `AddMenuItemL` updates was never seen to appear; per-language labels, whose old manifest-side sketch is void and which have no Rust-side design yet; and an `AddMenuItemL` that actually leaves, since no out-of-memory run was staged.
## 96. Localisation of the strings an application reads itself: a Rust table, and why nothing is cached (T5, Rust SDK)

- **Requires:** experiments 69 (the `no_std` hello and its 3 187 bytes), 72/80 (what an atomic costs on this CPU), 85 (`NanoTicks` = 1 000 µs inside EKA2L1), 91 (the compile-time guarantee a menu constant won). Design: [locale-rust-design.md](locale-rust-design.md).
- **Why:** localisation is two unrelated problems sharing a word. The launcher reads the application's *name* while the application is not running, so that half can only be a `localisable_resource_file` compiled per language. Everything **inside** the application is read by our own Rust and needs no Symbian resource at all — and the user had ruled out putting UI content in `symdev.toml`, which holds what the phone needs *before* the application runs. This experiment is the inside half only.
- **Procedure:** (1) re-verify `User::Language()` and the `TLanguage` enum from `euser.dso` and `e32const.h`; (2) `symbian-sys::euser::User_Language`, `symbian_core::locale::{Language, lang}`; (3) `symbian_std::locale!`, a `macro_rules!` whose expansion makes rustc do the checking; (4) `symbian-rs/examples/locale` through `symdev build`/`package`/`test --emulator`, run three times with the emulator set to three languages; (5) a throwaway `examples/sizeprobe` for the four size variants and the six compile errors.
- **Outcome:** pass (emulator only; no device, and the ROM offers six languages)
- **Evidence:** 2026-09-21, `symbian-rs/corpus/96-locale/` (the 9 684-byte E32, three `measured-lang*.txt`, `compile-errors.md`, `sizes.md`); source is `symbian-rs/examples/locale`.

  - **`User::Language()` is there and returns what the emulator is set to.** `nm -D epoc32/release/armv5/lib/euser.dso` → `00000a54 T _ZN4User8LanguageEv`, declared `IMPORT_C static TLanguage Language();` at `e32std.h:4531`. Three runs of one `localedemo.sisx`, changing only `language:` in `~/.local/share/EKA2L1/config.yml`: **1 → `ELangEnglish`, "Hello from Rust"; 2 → `ELangFrench`, "Bonjour depuis Rust", "langue", "d'accord"; 3 → `ELangGerman`, and since the table has no German the declared default answers, "Hello from Rust"**. `localedemo: 8 passed` all three times. `TLanguage` is 108 enumerators at `e32const.h:1439` (`ELangTest = 0`, `ELangEnglish = 1`, `ELangRussian = 16`, `ELangUkrainian = 93`, `ELangNone = 0xFFFF`); the header is not ISO text and needs `LC_ALL=C grep -a`.
  - **The emulator's language can be changed, but only to a language the ROM lists — and that bounds what can ever be tested here.** The key is `language:` (not `emulator-language:`, which is the Qt front-end's). EKA2L1 validates it against the device's `Z:\resource\bootdata\languages.txt` and silently rewrites the config to the ROM default otherwise (`src/emu/system/src/epoc.cpp` 1379-1383; the list is read in `src/emu/system/src/devices.cpp` around 320). RM-469's file, UTF-16LE, is `01,d 02 03 14 18 05` — **English (default), French, German, Turkish, Dutch, Italian, and nothing else**. `language: 93` came back as `language: 1` and `User::Language()` still answered 1. **Ukrainian, Russian and every dialect enumerator are untestable on this ROM.** They are exercised through `Text::get_in`, which takes a language instead of asking for one, and those six cases pass — but that is not evidence about a device *set* to `ELangEnglish_Apac`, and nothing claims it is.
  - **The declaration is Rust, and rustc does the checking.** `symbian_std::locale! { languages: english, ukrainian, french; GREETING = { english: "…", ukrainian: "…", french: "…" }, }` expands to a `pub struct Text` with **one field per declared language** plus one `pub const` per key. That single choice buys every guarantee for free: a **key missing from one language** is `error[E0063]: missing field ukrainian in initializer of Text`, because rustc already refuses an incomplete struct literal. A **mistyped key** is `error[E0425]: cannot find value GREETNG in module strings` with the "similarly named constant" note. A **language nobody declared** on a row is `E0560`. A **word that is not a `TLanguage`** is `E0425` in `locale::lang` with a `help` that fixes the spelling. Two rows with the same key is `E0428`; a non-string translation is `E0308`. All six transcripts are in the corpus.
  - **Why not `locales/uk.toml`.** The `rust-i18n` layout needs a reader, and a reader is a proc macro (which the menu slice measured: `symdev-manifest` plus `serde`, `toml`, `thiserror`, `syn` in every application's host build) or a `build.rs` plus an `include!` in every application — and then it must **re-implement** the "present in one file, missing from the other" check that the struct literal gives away. A `macro_rules!` is 90 lines in `symbian-std`, adds no dependency to anything, works under plain `cargo build`, and keeps every language for one key on adjacent lines, which is also where a human notices a missing one. If a `.toml` is ever really wanted, `include!("../locales/all.rs")` is the one-line escape.
  - **The fallback chain, and the two steps deliberately left out.** Exact language → the language a **dialect** is a dialect of → the declared default. Step two is not invented: the `_Apac`/`_Taiwan`/`_HongKong`/`_Prc`/`_Japan`/`_Thailand`/`Malay_Apac` suffixes are in `e32const.h`'s own enumerator names over languages the same enum declares, and they are exactly the seven it covers. Left out: **`TLocale::LanguageDowngrade(0..2)`** (`e32std.h:2278`, the licensee list `BaflUtils::NearestLanguageFile` walks) because reading it needs the `TLocale` layout and a `TLocale::Refresh` and `TODO: what TLocale::Refresh writes into iLanguageDowngrade on this ROM (not observed)`; and **the unsuffixed English variants** `ELangAmerican`, `ELangCanadianEnglish`, `ELangInternationalEnglish`, `ELangSouthAfricanEnglish`, `ELangAustralian`, `ELangNewZealand`, which are English to a reader and nothing in the header makes them English to a program, so `Language::base` answers `None` and an application that wants them adds the column.
  - **"Read the language once" is the wrong answer on this CPU, and the machine said so.** The obvious shape was built first — `User::Language()` once into a `static AtomicU32`, `u32::MAX` sentinel, relaxed load and store, benign race — and then measured. It costs **794 bytes**: ARMv5TE has no atomic instruction, `AtomicU32::load(Relaxed)` compiles to `bl __atomic_load_4` (disassembled: `82d0: bl 9678 <__atomic_load_4@plt>`), and linking that drags in `symbian-libcalls`' whole atomics object — thirty `__atomic_*` entry points, `AtomicLock`, `RFastLock::{CreateLocal,Wait,Signal}`, `__sync_synchronize`, 16 bytes of `.bss`, four more PLT entries, ~43 more dynamic symbols, `.text` +1 952. **And it is slower**: that libcall is a lock-based emulation, one `RFastLock` Wait/Signal pair per read, so 100 000 iterations take **16** nanokernel ticks through the cache and **12** straight to euser. So `Language::current()` asks euser every time, at **0.12 µs**, and the read-once the brief asked for is the *caller* holding the value: `Language` is two bytes and `Copy`, and `Text::get_in(language)` takes it. `core` has no `OnceCell` that avoids the atomic, there is no `std::sync` on this path, and the target JSON says `has-thread-local: false`.
  - **One language costs nothing.** A `hello`-shaped program: **3 187** bytes with the greeting as a plain `const`, **3 183** through a one-language `locale!`, 3 230 with a second language (19 of those 47 bytes are the French string), 3 309 with a third. The generated `get()` opens with a constant `if TRANSLATED.is_empty()`, LLVM folds the branch, and a single-language module never even imports `User::Language()`.
  - **Every other example is unchanged to the byte:** `alloc` 4 474, `async` 21 659, `atomics` 11 719, `files` 10 552, `hello` 3 187, `hello-raw` 752, `net` 13 379, `notes` 15 298, `query` 20 120, `shim` 4 474, `spawnee` 3 208, `time` 20 583, `tls` 16 272, `ui` 12 844, `ui-list` 14 489, against a baseline worktree at the same commit and the same path length. `localedemo` is **9 684** (it carries `alloc`, `String`, `core::fmt`, the file API and the report harness so the numbers can be read off the drive). The two `std` examples differ between the worktrees — `std-hello` 73 637 → 73 631, `std-net` 53 498 → 53 358 — and **it is not this branch**: with `pub mod locale;` commented out of both `symbian-core` and `symbian-std` and `euser.rs` reverted, `std-net` in this worktree still builds to 53 358. Each figure is reproducible inside its own tree and both trees' regenerated `Cargo.lock`s are identical; what differs was not determined. Experiment 91 recorded the same instability for `std-hello`.
