...
- **Evidence:** 2026-09-21, `symbian-rs/corpus/94-ui-list/` (the 14 489-byte E32 and four screenshots); source is `symbian-rs/examples/ui-list`.
  ...
  - **Sizes.** `listdemo` **14 489**. Unchanged **to the byte** by this work: `uidemo` 12 715, `hello` 3 187, `hello-raw` 752, `files` 10 552, `net` 13 379. The `-l:euser.dso -l:drtaeabi.dso`-before-the-Rust-archive ordering is untouched, and the list needed **no second `-u`**: the one call that runs from C++ into Rust is a function pointer in a table passed at create time, not a symbol the shim imports, so only `symrs_app_vtbl` still needs one.
