...
- **Outcome:** pass (emulator only; no device)
- **Evidence:** 2026-09-21, `symbian-rs/corpus/87-async/` (the 21 650-byte E32); source is `symbian-rs/examples/async`. `asyncdemo: 15 passed`, `symdev test --emulator` exit 0 on two consecutive runs.

  - **The criterion, measured.** Written by the application itself to `E:\symdev\async73\measured.txt` with `symbian_std::time::Instant`: one 300 ms sleep **312 ms**; two 300 ms sleeps **awaited together 312 ms**; the same two in sequence **625 ms**; a race between 100 ms and 20 s **109 ms**; twenty scheduler install/run/uninstall rounds 312 ms. 312 against 625 is the whole claim — both requests were outstanding at once. 300 reads as 312 because the system tick is 15 625 µs and `RTimer::After` rounds to a tick boundary (experiment 85).
  ...
  - **Sizes, against a baseline built from `main` (d277b9d) with the same `symdev` binary.** Unchanged: `hello` **3 187**, `hello-raw` **752**, `alloc` **4 474**, `shim` **4 474**, `files` **10 552**, `time` **20 583**. `net` 13 322 → **13 379** (the guard). `atomics` 11 582 → **11 588** with *identical* `.text`, `.rodata` and `.bss` sizes and an identical uncompressed image (0x49a0): the only differences in the ELF are branch offsets, because `symbian-std` gained a dependency and its crate disambiguator changed, so LTO laid the same code out differently — the effect experiment 78 recorded on `alloc` and did not diagnose. The new `async` is **21 650**. No new `DT_NEEDED`: `CActive`, `CActiveScheduler` and `RTimer` are all euser, and `operator delete` already came from `scppnwdl.dso`.
  - **Regression, all re-run in the emulator:** `files` 16 passed, `atomics` 23, `time` 29, `net` 22, `async` 15.
