...
- **Evidence:** 2026-09-21, `symbian-rs/corpus/86-ui/` (the 12 715-byte E32 and both screenshots); source is `symbian-rs/examples/ui`.
  ...
  - **The 107 KB did not come back.** `uidemo.exe` is **12 715 bytes**, of which 5 156 are the result-file harness (`symbian_std::fs` plus `core::fmt`); the drawing-and-keys application alone is **7 559**. Experiment 76's mixed C++/Rust probe was 107 028. The cause is the one experiment 77 found and this step did not touch: `-l:euser.dso -l:drtaeabi.dso` before the Rust archive resolves `mem*` from ROM, so the single `compiler_builtins` object is never pulled even with a 31 KB C++ object on the line. Every other example is unchanged **to the byte** — `hello` 3 187, `hello-raw` 752, `alloc` 4 474, `shim` 4 474, `files` 10 552, `atomics` 11 582, `time` 20 583 — so `symbian_std::ui` costs a console application nothing. (`net` is 13 322 against experiment 84's 13 183; it grew in `main`'s own `d277b9d`.)
