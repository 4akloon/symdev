...
- **Evidence:** 2026-09-21, `symbian-rs/corpus/88-tls/` (the 16 272-byte E32); source is `symbian-rs/examples/tls`. `tlsdemo: 45 passed`, `symdev test --emulator` exit 0.
  ...
  - **`has-thread-local` stays `false`, now with evidence rather than a note.** The field governs `#[thread_local]` statics, which need an ELF `PT_TLS` segment; the E32 image model has no such segment and `symdev-elf2e32` has no notion of one (`grep -rin 'tls\|tdata\|tbss'` over the crate matches only file names). Nothing in this slice uses compiler support, so setting it true would buy a feature the post-linker would have to reject.
