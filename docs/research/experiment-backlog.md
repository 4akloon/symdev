...
- **Evidence:** 2026-09-21, `symbian-rs/corpus/89-std/` (the 52 210-byte E32); source is `symbian-rs/examples/std-hello`. `stdhello: 20 passed`, `symdev test --emulator` exit 0.
  ...
  - **What is real, and what is `Unsupported` and why.**
    ...
    | `net` | — | **not done.** `sys/net` has no `unsupported` fallback to start from and a full `TcpStream`/`UdpSocket`/`LookupHost` backend is a slice of its own; `symbian_std::net` (experiment 84) is still the way to a socket, and a `std` application reaches it through the `no_std` facade. This is the one gate of step 77 that did not close |
  ...
  - **Open, and stated rather than left to be discovered:** `std::net` (the facade is still the way to a socket); `read_dir`; a drive letter as a `Path` prefix; `File` crossing threads, which the type system allows and the file server will answer `KErrBadHandle` to, because `RFs::ShareProtected` has not been observed; `process`/`env`/`args`, which are `Unsupported` by choice; and, as for every step so far, **no device has run any of this**.
