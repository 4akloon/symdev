...
- **Evidence:** 2026-09-21, `symbian-rs/corpus/91-ui-menu/` (`uidemo.exe` 12 844, the generated `uidemo.rss`/`uidemo.rsc` and four screenshots); source is `symbian-rs/examples/ui`.
  ...
  - **One run did not reproduce.** An early `F1 Down Return` left the application at `bars=6 keys=6 cmd=2` where the identical sequence gives `bars=2 keys=0 cmd=1` before and after; extra key events, not diagnosed, seen once in six runs. Worth knowing because it can make a correct build look broken.
