...
- `State.ui: Option<Ui>` → a `Ui` holding nulls until `construct`: ui −36, ui-list 0, notes −32, query −24, probe −39. Not kept: it makes a `Ui` with null handles representable (against `Ui::new`'s own safety contract) and leans on the call order alone for ~30 bytes.
