...
- Regression, all rebuilt and re-run in the emulator: `files` 16 passed, `atomics` 23, `time` 29, `net` **22 passed** with the new refusal in `blocking` (a console program installs no scheduler, so the guard never fires there).
- Sizes against a baseline built from `main` (d277b9d) with the same symdev binary: `hello` 3 187, `hello-raw` 752, `alloc` 4 474, `shim` 4 474, `files` 10 552, `time` 20 583 — all unchanged; `net` 13 322 -> 13 379 (+57, the guard, after `#[inline(never)]` took it from +132); `atomics` 11 582 -> 11 588 with **identical** `.text`/`.rodata`/`.bss` sizes and an identical uncompressed image (0x49a0) — a layout shuffle from `symbian-std`'s crate hash changing, the same effect experiment 78 saw on `alloc`. New: `async` **21 650**.
- Host gate green: `cargo test --workspace --offline`, `cargo clippy --workspace --all-targets --offline`, `cargo fmt --all --check`.
