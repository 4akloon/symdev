...
- Runtime, EKA2L1: `symdev test --emulator` → `uidemo: 3 passed`, 0 panics. `symdev run` + `emukey.py keys <pid> F1 Down Return` → screenshot reads `bars=2 keys=0 cmd=1` (experiment 91's exact result): `menu::FEWER` fired. 1 607 differing pixels, bbox (393,157)–(848,594) over the 900×600 window. Emulator I started killed with `kill -9`, gone.
