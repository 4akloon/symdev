...
/// The entry point of a Symbian application, because `lang_start` cannot be one.
///
/// On every other platform the C runtime calls `main`, rustc generates a call to
/// `lang_start`, and the linker ties the two together. Here the image's entry is the
/// C++-mangled `E32Main()` that `eexe.lib`'s startup reaches, and the application crate
/// is a `staticlib` in which rustc never looks for a `fn main` at all — so nothing
/// generates a `lang_start` call and `lang_start` itself is private.
///
/// `#[symbian_std::main]` writes the `E32Main` and calls this, which is `lang_start`
/// with the arguments a Symbian process does not have and with the one thing the
/// platform will not do for itself: running the main thread's thread-local
/// destructors. Nothing in this kernel runs a destructor for any thread (experiment
...
pub fn symbian_start<T: crate::process::Termination + 'static>(main: fn() -> T) -> i32 {
    ...
    crate::sys::thread::drop_thread_locals();
    code
}
