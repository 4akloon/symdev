...
pub fn exit(code: i32) -> ! {
    cfg_select! {
        ...
        target_os = "symbian" => {
            // `User::Exit(TInt)`, which is how a Symbian program ends on purpose and
            // what the creator reads back as `RProcess::ExitReason()`. The fallback
            // below is `intrinsics::abort()`, an undefined instruction that surfaces as
            // a bare `KERN-EXEC 3` — a crash, not an exit, and it would lose the code.
            //
            // SAFETY: a euser static taking one scalar and never returning. Nothing
            // after it runs, which is what `-> !` states.
            unsafe { symbian_sys::euser::User_Exit(code) }
        }
