...
pub(crate) mod guard {
    cfg_select! {
        ...
        any(target_os = "hermit", target_os = "xous", target_os = "symbian") => {
            // `std` is the only runtime, so it just calls the destructor functions
            // itself when the time comes.
            //
            // On Symbian nothing in the kernel runs a thread-local destructor: the
            // slot is a `TInt` the caller chooses and the kernel never walks it
            // (experiment 88). `crate::sys::thread::drop_thread_locals` sweeps the
            // keys and calls `crate::rt::thread_cleanup` at the end of every thread,
            // including the main one, which is why there is nothing to enable here.
            pub(crate) fn enable() {}
        }
...
pub(crate) mod key {
    cfg_select! {
        ...
        target_os = "symbian" => {
            ...
            mod symbian;
            pub(super) use racy::LazyKey;
            pub(super) use symbian::{Key, get, set};
            pub(crate) use symbian::run_dtors;
            use symbian::{create, destroy};
        }
        _ => {}
    }
}
