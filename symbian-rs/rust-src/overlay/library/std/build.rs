...
fn main() {
    ...
    if target_os == "linux"
        ...
        || target_os == "haiku"
        || target_os == "vxworks"
        // Symbian OS 9.3 (S60 3rd FP2): the platform layer in `sys/*/symbian` is real
        // (design spec §11 step 77), so `std` is not restricted here. What is genuinely
        // missing — `process`, `env`, `args`, `read_dir` — answers `Unsupported`, which
        // is a platform answer `std` already supports.
        || target_os == "symbian"
        || target_arch == "wasm32"
        || target_arch == "wasm64"
        || target_os == "espidf"
