...
impl UidCrc {
    ...
    pub fn parse_uid(token: &str) -> Option<u32> {
        let t = token.trim();
...
mod tests {
    ...
    #[test]
    fn parse_uid_accepts_recorded_hex_and_zero() {
        assert_eq!(UidCrc::parse_uid("0x1000007a"), Some(0x1000_007a));
        assert_eq!(UidCrc::parse_uid("0"), Some(0));
        assert_eq!(UidCrc::parse_uid("0xffffffff"), Some(0xffff_ffff));
    }
}
